...
# Bản đồ mặc định (map_factory_200)
python evaluate.py

# Chỉ định bản đồ và trọng số
python evaluate.py --map map_NVIDIA/map_01.txt --w1 0.4 --w2 0.6

# Tùy chỉnh FMM
python evaluate.py --map map_health_care/HC_01.txt --T_max 2000 --a 0.7

# Đọc run cuối của 1 file
python read_results.py results/map_01_eval.json --run -1

# Chỉ xem 1 solver
python read_results.py results/map_01_eval.json --run -1 --solver ACO

# In toàn bộ đường đi (không rút gọn)
python read_results.py results/map_01_eval.json --run -1 --full-path

# Ẩn phần đường đi, chỉ xem chỉ số
python read_results.py results/map_01_eval.json --no-path

# Đọc tất cả file trong results/ cùng lúc
python read_results.py

from read_results import get_path_coords

# Lấy path của Christofides từ run cuối
path = get_path_coords("results/map_01_eval.json", run_index=-1, solver="Christofides")
# → [(51, 133), (50, 133), (49, 133), ...]

path_aco = get_path_coords("results/map_01_eval.json", solver="ACO")

path_coords: [[row, col], [row, col], ...]   ← JSON list of list
get_path_coords() tự chuyển thành list of tuple khi đọc ra
