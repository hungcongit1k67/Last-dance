...
def bresenham_cells(start, end):
    ...
    centers. Each iteration advances along the major axis and conditionally
    along the minor axis using a single integer error term, so exactly one cell
    is visited per major-axis step (thin line, no corner doubling).
    """
    r, c = int(start[0]), int(start[1])
    r1, c1 = int(end[0]), int(end[1])

    cells = []  # danh sách các cell nằm trên đường Bresenham
    seen = set()
    _append_unique(cells, seen, (r, c))

    # Tính độ lệch (lấy trị tuyệt đối) và hướng di chuyển trên lưới
    dc = abs(c1 - c)  # độ lệch theo cột --> ngang
    dr = abs(r1 - r)  # độ lệch theo hàng --> dọc
    step_c = 1 if c1 > c else -1  # hướng theo cột
    step_r = 1 if r1 > r else -1  # hướng theo hàng

    if dc == 0 and dr == 0:
        return cells

    # err là sai số tích lũy (toàn số nguyên). Mỗi bước so sánh 2*err với -dr và dc
    # để quyết định bước theo cột, theo hàng, hay cả hai (bước chéo).
    err = dc - dr
    while (r, c) != (r1, c1):
        e2 = 2 * err
        if e2 > -dr:          # bước theo cột (trục ngang)
            err -= dr
            c += step_c
        if e2 < dc:           # bước theo hàng (trục dọc)
            err += dc
            r += step_r
        _append_unique(cells, seen, (r, c))
...
def draw_grid(matrix, output_path="E:\\last_dance\\LastDance\\FMF_new\\bresenham_grid.png", show=False):
