...
class ModifiedAStar:
    ...
    The g-cost uses weighted length, cumulative radiation risk and turning energy.
    Cells with radiation >= ri_max are treated as blocked when high-risk avoidance is enabled.
    """
    ...
    def heuristic(self, current: GridPosition, goal: GridPosition) -> float:
        # Conservative heuristic: straight-line length plus a small estimated risk.
    ...
    def search(self, start: GridPosition, goal: GridPosition) -> AStarResult:
        if not self.grid_map.is_passable(start, avoid_high_risk=self.high_risk_avoidance):
            return AStarResult([], {"length": float("inf"), "risk": float("inf"), "energy": float("inf"), "total": float("inf")}, False, 0)
        ...
        while open_heap:
            ...
            for nxt in self.grid_map.neighbors(current, avoid_high_risk=self.high_risk_avoidance):
                if nxt in closed:
                    continue
                step_cost, _, _, _ = weighted_segment_cost(self.grid_map, prev_pos, current, nxt, self.weights)
        ...
        return AStarResult([], {"length": float("inf"), "risk": float("inf"), "energy": float("inf"), "total": float("inf")}, False, expanded)
